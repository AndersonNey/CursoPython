...
def main(main_imagens_folder, new_width=800):
    ...
    for root , dirs , files in os.walk(main_imagens_folder):
        for file in files:
            ...
            if 'exif' in img_pillow.info:  #se existe metadados insira eles na foto
                new_image.save(
                    new_file_full_path,
                    optimize=True,
                    quality=70,
                    exif = img_pillow.info['exif']   #colocando os metadados na foto convertida
                )
            ...
            #CUIDADO VAI APAGAR OS ARQUIVOS NEM PRA LIXEIRA VAI##############################
            #os.remove(file_full_path)
            #################################################################################
